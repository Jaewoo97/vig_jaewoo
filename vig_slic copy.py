...
class Stem_slic_V2(nn.Module):
    ...
    def compute_features(self, imgs, segmaps, imgs_convFeat):
        ...
        imgs_convFeat_idx = imgs_idxs.repeat(batchSize, 1, 1)
        imgs_convFeat_idx = imgs_convFeat_idx.unsqueeze(1).expand(-1, featDim, -1, -1)
        imgs_convFeat_idx = imgs_convFeat_idx.flatten(-2, -1)
        ...
        idxed_feat = idxed_feat.unsqueeze(-1).expand(-1, -1, -1, -1, self.numSegments)
        idxed_feat = (idxed_feat * masks_resized.unsqueeze(1).float()).sum(dim=(2, 3))
        masks_resized = masks_resized.sum(dim=(1,2))
    ...
    def slic_init(self, x, org_x, x_convFeat):
        batchSize = len(x)
        ...
        x_out = self.compute_features(x, sliced, x_convFeat)        # TODO: slic에 따라서 pooling 해서 feature 계산
    ...
    def forward(self, x, org_x):
        ...
        # pdb.set_trace()
        x_slic, x = self.slic_init(x, org_x, x_convFeat)        # x_slic: B, 64, 64 / x: B, 11, 14, 14
        # pdb.set_trace()
        # x = self.convs_slic(x)
