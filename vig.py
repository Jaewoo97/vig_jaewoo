...
class Stem_tiny_14by14(nn.Module):     # 14 by 14
    """ Image to Visual Word Embedding
    Overlap: https://arxiv.org/pdf/2106.13797.pdf
    """
    def __init__(self, img_size=224, in_dim=3, out_dim=768, act='relu'):
...
class Stem_tiny_10by10(nn.Module):     # 10 by 10
    def __init__(self, img_size=224, in_dim=3, out_dim=768, act='relu'):
        super().__init__()
        self.convs = nn.Sequential(
            nn.Conv2d(in_dim, out_dim//8, 3, stride=2, padding=1),
            ...
            nn.BatchNorm2d(out_dim//4),
            act_layer(act),
            nn.Conv2d(out_dim//4, out_dim//2, 3, stride=1, padding=0),
            nn.BatchNorm2d(out_dim//2),
            act_layer(act),
            nn.Conv2d(out_dim//2, out_dim, 3, stride=1, padding=0),
            nn.BatchNorm2d(out_dim),
            act_layer(act),
            nn.Conv2d(out_dim, out_dim, 3, stride=1, padding=0),
            nn.BatchNorm2d(out_dim),
...
class DeepGCN_14by14(torch.nn.Module):
    def __init__(self, opt):
        super(DeepGCN_14by14, self).__init__()
        ...
        self.stem_tiny = Stem_tiny_14by14(out_dim=channels, act=act)
...
class DeepGCN_10by10(torch.nn.Module):
    def __init__(self, opt):
        super(DeepGCN_10by10, self).__init__()
        channels = opt.n_filters
        k = opt.k
        act = opt.act
        norm = opt.norm
        ...
        self.stem = Stem(out_dim=channels, act=act)
        self.stem_tiny = Stem_tiny_10by10(out_dim=channels, act=act)

        dpr = [x.item() for x in torch.linspace(0, drop_path, self.n_blocks)]  # stochastic depth decay rule 
        print('dpr', dpr)
        num_knn = [int(x.item()) for x in torch.linspace(k, 2*k, self.n_blocks)]  # number of knn's k
        print('num_knn', num_knn)
        max_dilation = 100 // max(num_knn)
        
        self.pos_embed = nn.Parameter(torch.zeros(1, channels, 10, 10))
...
@register_model
def vig_ti_64_gelu_10by10(pretrained=False, **kwargs):         # 230525 baseline
    class OptInit:
    ...
    opt = OptInit(**kwargs)
    model = DeepGCN_10by10(opt)
    model.default_cfg = default_cfgs['gnn_patch16_64']
    return model

@register_model
def vig_ti_64_gelu_14by14(pretrained=False, **kwargs):         # 230525 baseline
    class OptInit:
        def __init__(self, num_classes=200, drop_path_rate=0.0, drop_rate=0.0, num_knn=9, **kwargs):
            self.k = num_knn # neighbor num (default:9)
            self.conv = 'mr' # graph conv layer {edge, mr}
