...
def find_images_and_targets(folder, types=IMG_EXTENSIONS, class_to_idx=None, leaf_name_only=True, sort=True):
    ...
    for root, subdirs, files in os.walk(folder, topdown=False):
        ...
        # label = os.path.basename(rel_path) if leaf_name_only else rel_path.replace(os.path.sep, '_')      # full imagenet
        label = os.path.split(rel_path)[0] if leaf_name_only else rel_path.replace(os.path.sep, '_')        # Tiny imagenet
        for f in files:
...
def find_images_and_targets_val(folder, types=IMG_EXTENSIONS, class_to_idx=None, train_class_to_idx=None, leaf_name_only=True, sort=True):
    labels = []
    filenames = []
    ...
    for root, subdirs, files in os.walk(folder, topdown=False):
        for f in files:
            ...
            if ext.lower() in types:
                filenames.append(os.path.join(root, f))
                labels.append(train_class_to_idx[class_to_idx[f]])
            else: raise Exception("Validation image format not supported")
    if class_to_idx is None:
        # building class index
        unique_labels = set(labels)
        sorted_labels = list(sorted(unique_labels, key=natural_key))
...
def load_class_map(filename, root=''):      # full imagenet
...
def load_class_map_tiny(filename, root=''):      # tiny imagenet, map val_##.JPEG => n#########
    class_map_path = filename
    if not os.path.exists(class_map_path):
        class_map_path = os.path.join(root, filename)
        assert os.path.exists(class_map_path), 'Cannot locate specified class map file (%s)' % filename
    ...
    if class_map_ext == '.txt':
        with open(class_map_path) as f:
            class_to_idx = {v.split('\t')[0]: v.split('\t')[1] for v in f}
    else:
        assert False, 'Unsupported class map extension'
    return class_to_idx
...
class Dataset(data.Dataset):
    ...
    def __init__(
        ...
        class_to_idx = None
        if class_map:
            class_to_idx = load_class_map_tiny(class_map, root)
        if not train_class_to_idx:  # Train
            images, class_to_idx = find_images_and_targets(root, class_to_idx=class_to_idx)
        else:
            images, class_to_idx = find_images_and_targets_val(root, class_to_idx=class_to_idx, train_class_to_idx=train_class_to_idx)
        if len(images) == 0:
    ...
    def __getitem__(self, index):
        ...
        if self.transform is not None:
            ...
            elif self.transform[0] is not None:
                originalImg = self.transform[0](img)
                img = self.transform[1](originalImg.copy())
            else: 
                originalImg = img.copy()
                img = self.transform[1](originalImg)
                
        if target is None:
            target = torch.zeros(1).long()
        # pdb.set_trace()
        return img, target, np.asarray(originalImg).astype('uint8')
